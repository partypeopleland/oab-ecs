...
if [ -n "$STATE_BUCKET" ] && aws s3 cp "s3://$STATE_BUCKET/$UV_CACHE_KEY" /tmp/uv.tar.gz --quiet 2>/dev/null; then
  echo "✓ 從 S3 快取載入 uv"
else
  ...
  if [ -n "$STATE_BUCKET" ]; then
    aws s3 cp /tmp/uv.tar.gz "s3://$STATE_BUCKET/$UV_CACHE_KEY" --quiet || true
  fi
fi
...
aws s3 sync "s3://$STATE_BUCKET/shared/common/" "$HOME/" || true
